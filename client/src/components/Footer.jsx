...
export default function Footer() {
  ...
  return (
    <footer className="bg-bg-2 border-t border-white/[0.08] relative overflow-hidden">
      {/* glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[200px] bg-[radial-gradient(ellipse,rgba(212,255,0,0.06)_0%,transparent_70%)] pointer-events-none" />
      ...
      <div className="container relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 py-16">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-2.5 font-display text-xl font-bold uppercase mb-4">
              <Dumbbell size={26} strokeWidth={2.5} className="text-accent" />
            ...
            <div className="flex gap-3">
              {[
                ...
                <a key={label} href="#" aria-label={label}
                  className="w-10 h-10 rounded-full border border-white/[0.08] flex items-center justify-center text-muted transition-all duration-200 hover:bg-accent hover:border-accent hover:text-black hover:-translate-y-1">
                  <Icon size={16} />
                </a>
              ))}
            </div>
          ...
          {/* Quick Links */}
          <div>
            <h4 className="text-xs font-bold tracking-[2px] uppercase text-gray-50 mb-5">Quick Links</h4>
            <ul className="flex flex-col gap-2.5">
              {quickLinks.map(({ to, label }) => (
                <li key={to}>
                  <Link to={to} className="flex items-center gap-1.5 text-sm text-muted transition-all duration-200 hover:text-accent hover:gap-2.5 group">
          ...
          {/* Contact */}
          <div>
            <h4 className="text-xs font-bold tracking-[2px] uppercase text-gray-50 mb-5">Get In Touch</h4>
            <ul className="flex flex-col gap-3.5 mb-6">
              {[
                ...
                <li key={lines[0]} className="flex items-start gap-2.5">
